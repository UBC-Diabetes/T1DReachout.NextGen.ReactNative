...
import { observeSavedPosts } from '../../HomeView/savedPostsHelpers';
...
const DiscussionHomeView: React.FC = ({ route, theme }) => {
	...
	// Subscribe to saved posts updates when Saved Posts tab is active
	useEffect(() => {
		if (selectedTab === DiscussionTabs.SAVED_POSTS) {
			const subscription = observeSavedPosts(999, posts => {
		...
		<View style={styles.mainContainer}>
			...
			<View style={{ width: '100%', flex: 1, backgroundColor: themeColors.nextGenBackground }}>
				...
				{selectedTab === DiscussionTabs.SAVED_POSTS && (
					<FlatList
						data={starredPosts}
						renderItem={({ item }) => <SavedPostCard post={item} theme={theme} server={server} />}
						keyExtractor={item => item._raw.id}
